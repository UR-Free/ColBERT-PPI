"""The single command-line entry point; task defaults live in config/*.json."""
...
COMMANDS = {
    "download": ("commands.download", "run", "Download weights or benchmark data"),
    "prepare": ("commands.prepare", "run", "Convert single-chain PDB pairs to model inputs"),
    "predict": ("commands.predict", "run", "Predict scores for your protein pairs"),
    "evaluate": ("commands.evaluate", "run", "Evaluate a released model on a benchmark"),
    "train": ("training", "train", "Train on the small contact-labelled examples"),
    "reproduce": ("commands.reproduce", "run", "Recalculate paper metrics from saved predictions"),
    "plot": ("commands.plot", "run", "Plot the supplied paper evidence"),
    "example": ("commands.example", "run", "Check scoring against one frozen-vector example"),
}


def build_parser():
    parser = argparse.ArgumentParser(prog="colbert-ppi", description="Protein partner retrieval with ColBERT-PPI")
    ...
    for command, (_, _, description) in COMMANDS.items():
        p = subparsers.add_parser(command, help=description, description=description)
...
def main(argv=None):
    ...
    try:
        ...
        if args.dry_run:
            ...
            return
        module, function, _ = COMMANDS[args.command]
        with working_directory(args.root):
            run = getattr(importlib.import_module(f"colbert_ppi.{module}"), function)
            run(args)
    except (OSError, ValueError, RuntimeError, ImportError) as error:
        parser.exit(1, f"colbert-ppi: {error}\n")

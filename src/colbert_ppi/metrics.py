...
def check_localisation(root, output_dir):
    ...
    for arm, method in [
        ...
        data = np.load(root / f"localisation/{arm}.npz")
...
def check_protein_rna(root, output_dir):
    ...
    labels = np.load(root / "pri/labels.npz")
    cohorts = [k[:-9] for k in labels.files if k.endswith("_positive")]
    pri = []
    for meta in json.loads((root / "pri/methods.json").read_text()):
        s = np.load(root / "pri" / meta["file"])["scores"]
